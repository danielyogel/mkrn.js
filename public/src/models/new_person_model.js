...
export function handleAge(age) {
    ...
    if (empty_age) {
        objectAssignThenEmit(age_state, {value: '', error: ''});
    } else {
        if (is_valid) {
            objectAssignThenEmit(age_state, {value: age, error: ''});
        } else {
            objectAssignThenEmit(age_state, {value: age, error: 'age must be more then 3'});
        }
    }
}
